 ...
 <div class="content">
  ...
  <div class="row">
   <div class="col-md-12">
    <div class="panel_s">
     <div class="panel-body">
      <h4 class="no-margin">
       ...
       <br />
       <small>
        <a href="<?php echo site_url(ADVICES_SLUG.$article->slug); ?>" target="_blank"><?php echo site_url(ADVICES_SLUG.$article->slug); ?></a>
      </small>
      <?php } ?>
    </h4>
    <?php if(isset($article)){ ?>
    <p>
    <small>
       <?php echo _l('article_total_views'); ?>: <?php echo total_rows('tblviewstracking',array('rel_type'=>'advices_article','rel_id'=>$article->articleid)); ?>
     </small>
     <?php if(has_permission('advices','','create')){ ?>
     <a href="<?php echo admin_url('advices/article'); ?>" class="btn btn-success pull-right"><?php echo _l('kb_article_new_article'); ?></a>
     <?php } ?>
     <?php if(has_permission('advices','','delete')){ ?>
     <a href="<?php echo admin_url('advices/delete_article/'.$article->articleid); ?>" class="btn btn-danger _delete pull-right mright5"><?php echo _l('delete'); ?></a>
     <?php } ?>
   ...
   <?php if(has_permission('advices','','create')){
     echo render_select_with_input_group('articlegroup',get_advices_groups(),array('groupid','name'),'kb_article_add_edit_group',$value,'<a href="#" onclick="new_advices_group();return false;"><i class="fa fa-plus"></i></a>');
   } else {
    echo render_select('articlegroup',get_advices_groups(),array('groupid','name'),'kb_article_add_edit_group',$value);
  }
  ?>
...
<?php if((has_permission('advices','','create') && !isset($article)) || has_permission('advices','','edit') && isset($article)){ ?>
