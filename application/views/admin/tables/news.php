...
$where   = [];
$filter  = [];
$groups  = $this->ci->news_model->get_kbg();
$_groups = [];
...
if (!has_permission('news', '', 'create') && !has_permission('news', '', 'edit')) {
    array_push($where, ' AND news.active=1');
}
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'articlegroup') {
            ...
            $link = site_url(NEWS_SLUG . $aRow['slug']);
            $_data = '<b>' . $_data . '</b>';
            if (has_permission('news', '', 'edit')) {
                $_data = '<a href="'.admin_url('news/article/' . $aRow['articleid']).'" class="font-size-14">' . $_data . '</a>';
            } else {
            ...
            if (has_permission('news', '', 'edit')) {
                $_data .= ' | <a href="'.admin_url('news/article/' . $aRow['articleid']).'">' . _l('edit') . '</a>';
            }

            if (has_permission('news', '', 'delete')) {
                $_data .= ' | <a href="'.admin_url('news/delete_article/' . $aRow['articleid']).'" class="_delete text-danger">' . _l('delete') . '</a>';
            }
