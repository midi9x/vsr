<div class="modal fade" id="property_characteristics_modal" tabindex="-1" role="dialog">
    <div class="modal-dialog">
        <?php echo form_open(admin_url('property/characteristic'),array('id'=>'characteristic_form')); ?>
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">
                    <span class="edit-title"><?php echo _l('edit_property_characteristic'); ?></span>
                    <span class="add-title"><?php echo _l('new_characteristic'); ?></span>
                </h4>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <?php echo render_input('characteristic_name','name'); ?>
                        <div id="property_slug" class="hide">
                            <?php echo render_input('characteristic_slug', 'slug'); ?>
                        </div>
...
<script>
    ...
    $('#property_characteristics_modal').on("hidden.bs.modal", function(event) {
        ...
        $('#additional').html('');
        $('#property_characteristics_modal input').not('[type="hidden"]').val('');
        $('#property_characteristics_modal textarea').val('');
        $('.add-title').removeClass('hide');
        $('.edit-title').removeClass('hide');
        $('#property_characteristics_modal input[name="characteristic_order"]').val($('table tbody tr').length + 1);
    });
});
function manage_characteristics(form) {
...
function new_characteristic() {
    $('#property_characteristics_modal').modal('show');
    $('.edit-title').addClass('hide');
}

function edit_characteristic(invoker, id) {
