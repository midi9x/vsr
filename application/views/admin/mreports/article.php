 ...
 <div class="content">
  ...
  <div class="row">
   <div class="col-md-12">
    <div class="panel_s">
     <div class="panel-body">
      <h4 class="no-margin">
       ...
       <br />
       <small>
        <a href="<?php echo site_url(MREPORTS_SLUG.$article->slug); ?>" target="_blank"><?php echo site_url(MREPORTS_SLUG.$article->slug); ?></a>
      </small>
      <?php } ?>
    </h4>
    <?php if(isset($article)){ ?>
    <p>
    <small>
       <?php echo _l('article_total_views'); ?>: <?php echo total_rows('tblviewstracking',array('rel_type'=>'mreports_article','rel_id'=>$article->articleid)); ?>
     </small>
     <?php if(has_permission('mreports','','create')){ ?>
     <a href="<?php echo admin_url('mreports/article'); ?>" class="btn btn-success pull-right"><?php echo _l('kb_article_new_article'); ?></a>
     <?php } ?>
     <?php if(has_permission('mreports','','delete')){ ?>
     <a href="<?php echo admin_url('mreports/delete_article/'.$article->articleid); ?>" class="btn btn-danger _delete pull-right mright5"><?php echo _l('delete'); ?></a>
     <?php } ?>
   ...
   <?php if(has_permission('mreports','','create')){
     echo render_select_with_input_group('articlegroup',get_mreports_groups(),array('groupid','name'),'kb_article_add_edit_group',$value,'<a href="#" onclick="new_mreports_group();return false;"><i class="fa fa-plus"></i></a>');
   } else {
    echo render_select('articlegroup',get_mreports_groups(),array('groupid','name'),'kb_article_add_edit_group',$value);
  }
  ?>
...
<?php if((has_permission('mreports','','create') && !isset($article)) || has_permission('mreports','','edit') && isset($article)){ ?>
