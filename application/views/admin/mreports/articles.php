<?php init_head();
$has_permission_edit = has_permission('mreports','','edit');
$has_permission_create = has_permission('mreports','','create');
?>
<div id="wrapper">
  <div class="content">
    <div class="row">
         ...
         <div class="panel-body">
           <div class="_buttons">
             <?php if($has_permission_create){ ?>
               <a href="<?php echo admin_url('mreports/article'); ?>" class="btn btn-info mright5"><?php echo _l('kb_article_new_article'); ?></a>
               <?php } ?>
               <?php if($has_permission_edit || $has_permission_create){ ?>
                 <a href="<?php echo admin_url('mreports/manage_groups'); ?>" class="btn btn-info mright5"><?php echo _l('als_kb_groups'); ?></a>
                 <?php } ?>
             ...
             <div class="row">
               <div class="tab-content">
                 <div role="tabpanel" class="tab-pane kb-kan-ban kan-ban-tab" id="kan-ban">
                     ...
                     <ul class="kan-ban-col<?php if(!$has_permission_edit){echo ' sortable-disabled'; } ?>" data-col-group-id="<?php echo $group['groupid']; ?>">
                       <li class="kan-ban-col-wrapper">
                        <div class="border-right panel_s">
                         <div class="panel-heading-bg primary-bg" style="background: #000" data-group-id="<?php echo $group['groupid']; ?>">
                          <?php if($has_permission_edit){ ?>
                            <i class="fa fa-reorder pointer"></i> <?php } ?>
                            <a href="#" class="color-white" <?php if($has_permission_create || $has_permission_edit){ ?>onclick="edit_kb_group(this,<?php echo $group['groupid']; ?>); return false;" data-name="<?php echo $group['name']; ?>" data-slug="<?php echo $group['group_slug']; ?>" data-description="<?php echo clear_textarea_breaks($group['description']); ?>" data-order="<?php echo $group['group_order']; ?>" data-active="<?php echo $group['active']; ?>" <?php } ?>><?php echo $group['name']; ?></a>
                            <small> - <?php echo total_rows('mreports','articlegroup='.$group['groupid']); ?></small>
                            </div>
                            <?php
                            $this->db->select('*, (SELECT COUNT(*) FROM tblviewstracking WHERE rel_type="mreports_article" AND rel_id=mreports.articleid) as total_views')->from('mreports')->where('articlegroup',$group['groupid'])->order_by('article_order','asc');
                               ...
                               <ul class="sortable article-group groups<?php if(!$has_permission_edit){echo 'sortable-disabled'; } ?>" data-group-id="<?php echo $group['groupid']; ?>">
                                <?php foreach($articles as $article) { ?>
                                  <li class="<?php if($article['active'] == 0){echo 'line-throught';} ?>" data-article-id="<?php echo $article['articleid']; ?>">
                                   <div class="panel-body">
                                          <a href="<?php echo site_url(MREPORTS_SLUG.$article['slug']); ?>" target="_blank">
                                           <?php echo $article['subject']; ?></a>
                                            <?php if($has_permission_edit){ ?>
                                              <a href="<?php echo admin_url('mreports/article/'.$article['articleid']); ?>" target="_blank" class="pull-right"><span><i class="fa fa-pencil-square-o" aria-hidden="true"></i></span></a>
                                              <?php } ?>
                  ...
                  <?php include_once(APPPATH.'views/admin/mreports/group.php'); ?>
                  <?php init_tail(); ?>
                  <script>
                   ...
                   $(function(){
                     ...
                     update: function(event, ui) {
                       if (this === ui.item.parent()[0]) {
                         ...
                         setTimeout(function() {
                           $.post(admin_url + 'mreports/update_kan_ban', {
                             order: order,
                     ...
                     $(".container-fluid").sortable({
                       ...
                       update: function(event, ui) {
                         ...
                         var data = {}
                         data.order = order;
                         $.post(admin_url + 'mreports/update_groups_order', data);
                       }
                     });
