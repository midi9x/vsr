<div class="modal fade" id="mreports_group_modal" tabindex="-1" role="dialog">
    <div class="modal-dialog">
        <?php echo form_open(admin_url('mreports/group'),array('id'=>'mreports_group_form')); ?>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <?php echo render_input('group_order','kb_group_order',total_rows('mreportsgroups') + 1,'number'); ?>
...
<script>
    ...
    _validate_form($('#mreports_group_form'), {
        name: 'required'
    }, manage_kb_groups);

    // On hidden modal reset the values
    $('#mreports_group_modal').on("hidden.bs.modal", function(event) {
        $('#kb_group_slug').addClass('hide');
        $('#kb_group_slug input').rules('remove', 'required');
        $('#additional').html('');
        $('#mreports_group_modal input').not('[type="hidden"]').val('');
        $('#mreports_group_modal textarea').val('');
        $('.add-title').removeClass('hide');
        $('.edit-title').removeClass('hide');
        $('#mreports_group_modal input[name="group_order"]').val($('table tbody tr').length + 1);
    });
});
...
function manage_kb_groups(form) {
    ...
    var url = form.action;
    var articleAddEdit = $('body').hasClass('mreports-article');
    if(articleAddEdit) {
    ...
    $.post(url, data).done(function(response) {
        if(!articleAddEdit) {
           window.location.reload();
            ...
            $('#mreports_group_modal').modal('hide');
        }
    });
    return false;
}
...
function new_mreports_group() {
    $('#mreports_group_modal').modal('show');
...
function edit_kb_group(invoker, id) {
    ...
    $('#mreports_group_modal input[name="name"]').val($(invoker).data('name'));
    $('#mreports_group_modal textarea[name="description"]').val($(invoker).data('description'));
    $('#mreports_group_modal input[name="group_order"]').val($(invoker).data('order'));
