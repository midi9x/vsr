...
class Advices_model extends CRM_Model
{
    ...
    {
        $this->db->select('slug,articleid, articlegroup, subject,advices.description,advices.active as active_article, avatar, datecreated, seo_title, seo_description, advicesgroups.active as active_group,name as group_name, group_slug, group_seo_title, group_seo_description, location_name,location_slug,advices.location_id as location_id, author');
        $this->db->from('advices');
        $this->db->join('advicesgroups', 'advicesgroups.groupid = advices.articlegroup', 'left');
        $this->db->join('property_locations', 'advices.location_id = property_locations.location_id', 'left');
        if ($this->router->directory  != 'admin/') {
            $this->db->where('advices.active', 1);
            $this->db->where('advicesgroups.active', 1);
        }
        if (is_numeric($id)) {
        ...
        if ($location != '') {
            if (is_numeric($location)) {
                $this->db->where('advices.location_id', $location);
            } else {
        ...
        if ($q != '') {
            ...
            $this->db->or_like('advices.description', $q);
        }
    ...
    public function get_related_articles($current_id, $customers = true)
    {
        ...
        $article = $this->db->get('advices')->row();

        $this->db->select('slug,articleid, articlegroup, subject,advices.description,advices.active as active_article, avatar, datecreated, seo_title, seo_description, advicesgroups.active as active_group,name as group_name, group_slug');
        $this->db->from('advices');
        $this->db->join('advicesgroups', 'advicesgroups.groupid = advices.articlegroup', 'left');
    ...
    public function add_article($data)
    {
        ...
        $slug_total = $this->db->count_all_results('advices');
        if ($slug_total > 0) {
        ...
        if ($insert_id) {
            logActivity('advice Article Added [ArticleID: ' . $insert_id . ' GroupID: ' . $data['articlegroup'] . ']');
        }
    ...
    public function delete_article($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            // $this->db->where('rel_type', 'advices_article');
    ...
    public function get_kbg($id = '', $active = '')
    {
        ...
        if (is_numeric($id)) {
            ...
            return $this->db->get('advicesgroups')->row();
        }
        $this->db->order_by('group_order', 'asc');

        return $this->db->get('advicesgroups')->result_array();
    }

    /**
     * Add advice knowledge base group/folder
    ...
    public function add_group($data)
    {
        ...
        $slug_total = $this->db->count_all_results('advicesgroups');
        if ($slug_total > 0) {
            $data['group_slug'] .= '-' . ($slug_total + 1);
        }

        $this->db->insert('advicesgroups', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            logActivity('advice Article Group Added [GroupID: ' . $insert_id . ']');
    ...
    public function update_group($data, $id)
    {
        ...
        $this->db->where('groupid', $id);
        $this->db->update('advicesgroups', $data);
        if ($this->db->affected_rows() > 0) {
    ...
    public function delete_group($id)
    {
        ...
        if (is_reference_in_table('articlegroup', 'advices', $id)) {
            return [
                'referenced' => true,
            ];
        }
        $this->db->where('groupid', $id);
        $this->db->delete('advicesgroups');
        if ($this->db->affected_rows() > 0) {
