...
class Laws_model extends CRM_Model
{
    ...
    public function get_related_articles($current_id, $customers = true)
    {
        ...
        return $this->db->get('laws')->result_array();
    }
    ...
    public function add_article($data)
    {
        ...
        $slug_total = $this->db->count_all_results('laws');
        if ($slug_total > 0) {
    ...
    public function delete_article($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            // $this->db->where('rel_type', 'laws_article');
    ...
    public function get_kbg($id = '', $active = '')
    {
        ...
        if (is_numeric($id)) {
            ...
            return $this->db->get('lawsgroups')->row();
        }
        $this->db->order_by('group_order', 'asc');

        return $this->db->get('lawsgroups')->result_array();
    }
    ...
    public function add_group($data)
    {
        ...
        $slug_total = $this->db->count_all_results('lawsgroups');
        if ($slug_total > 0) {
            $data['group_slug'] .= '-' . ($slug_total + 1);
        }

        $this->db->insert('lawsgroups', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
    ...
    public function update_group($data, $id)
    {
        ...
        $this->db->where('groupid', $id);
        $this->db->update('lawsgroups', $data);
        if ($this->db->affected_rows() > 0) {
    ...
    public function delete_group($id)
    {
        ...
        if (is_reference_in_table('articlegroup', 'laws', $id)) {
            return [
                'referenced' => true,
            ];
        }
        $this->db->where('groupid', $id);
        $this->db->delete('lawsgroups');
        if ($this->db->affected_rows() > 0) {
