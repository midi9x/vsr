...
function _maybe_system_setup_warnings()
{
    ...
    // Check php version
    add_action('before_start_render_dashboard_content', '_maybe_php_version_not_correct');
    // Some hosting providers cast this file as a malicious and may be deleted
    add_action('before_start_render_dashboard_content', '_maybe_tcpdf_file_is_missing');
}

function _maybe_php_version_not_correct(){
    if (phpversion() < "5.5") {
      // TODO
    }
}
...
function _maybe_static_index_html_file_exists() {
    if(file_exists(FCPATH.'index.html')) {
        ?>
        <div class="col-md-12">
...
function _is_cron_setup_required()
{
    if (get_option('cron_has_run_from_cli') == 0) {
        if (is_admin()) {
            ...
            $feature = total_rows('tblleadsintegration', [
                'active' => 1,
            ]);
            $using_cron_features += $feature;
            ...
            $feature = total_rows('tblinvoices', [
                'recurring >' => 0,
            ]);
            $using_cron_features += $feature;
            if ($feature > 0) {
            ...
            $feature = total_rows('tblexpenses', [
                'recurring' => 1,
            ]);
            $using_cron_features += $feature;
            if ($feature > 0) {
            ...
            $feature = total_rows('tblstafftasks', [
                'recurring' => 1,
            ]);
            $using_cron_features += $feature;
            if ($feature > 0) {
            ...
            $feature = total_rows('tbldepartments', [
                'host !='     => '',
                'password !=' => '',
                'email !='    => '',
            ]);
