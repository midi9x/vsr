...
class Surveys_model extends CRM_Model
{
    ...
    {
        $this->db->insert('tblsurveysendlog', [
    ...
    {
        $this->db->insert('tblsurveyresultsets', [
            'date'      => date('Y-m-d H:i:s'),
            'surveyid'  => $id,
    ...
    {
        $this->db->insert('tblformquestionboxesdescription', [
            'questionid'  => $questionid,
            'boxid'       => $boxid,
    ...
    {
        $this->db->insert('tblformquestions', [
            'rel_id'   => $surveyid,
            'rel_type' => 'survey',
            'question' => $question,
        ]);
    ...
    {
        $this->db->insert('tblformquestionboxes', [
            'boxtype'    => $type,
            'questionid' => $questionid,
        ]);
    ...
    public function update_question($data)
    {
        ...
        $this->db->update('tblformquestions', [
        ...
        if (isset($data['boxes_description'])) {
            foreach ($data['boxes_description'] as $box_description) {
                ...
                $this->db->update('tblformquestionboxesdescription', [
    ...
    public function add_mail_list($data)
    {
        ...
        if (isset($custom_fields)) {
            foreach ($custom_fields as $field) {
                if (!empty($field)) {
                    $this->db->insert('tblmaillistscustomfields', [
                        'listid'    => $listid,
                        'fieldname' => $field,
    ...
    public function update_mail_list($data, $id)
    {
        ...
        if (isset($data['list_custom_fields_update'])) {
            foreach ($data['list_custom_fields_update'] as $key => $update_field) {
                ...
                $this->db->update('tblmaillistscustomfields', [
    ...
    {
        $exists = total_rows('tbllistemails', [
            'email'  => $data['email'],
            'listid' => $data['listid'],
        ]);
        if ($exists > 0) {
        ...
        if ($insert_id) {
            if (isset($data['customfields'])) {
                foreach ($data['customfields'] as $key => $val) {
                    $this->db->insert('tblmaillistscustomfieldvalues', [
                        'listid'        => $data['listid'],
                        'customfieldid' => $key,
                        'emailid'       => $insert_id,
                        'value'         => $val,
