...
class Mreports_model extends CRM_Model
{
    ...
    {
        $this->db->select('slug,articleid, articlegroup, subject,mreports.description,mreports.active as active_article, avatar, datecreated, seo_title, seo_description, mreportsgroups.active as active_group,name as group_name, group_slug, group_seo_title, group_seo_description, location_name,location_slug,mreports.location_id as location_id, author');
        $this->db->from('mreports');
        $this->db->join('mreportsgroups', 'mreportsgroups.groupid = mreports.articlegroup', 'left');
        $this->db->join('property_locations', 'mreports.location_id = property_locations.location_id', 'left');
        if ($this->router->directory  != 'admin/') {
            $this->db->where('mreports.active', 1);
            $this->db->where('mreportsgroups.active', 1);
        }
        if (is_numeric($id)) {
        ...
        if ($location != '') {
            if (is_numeric($location)) {
                $this->db->where('mreports.location_id', $location);
            } else {
        ...
        if ($q != '') {
            ...
            $this->db->or_like('mreports.description', $q);
        }
    ...
    public function get_related_articles($current_id, $customers = true)
    {
        ...
        $article = $this->db->get('mreports')->row();

        $this->db->select('slug,articleid, articlegroup, subject,mreports.description,mreports.active as active_article, avatar, datecreated, seo_title, seo_description, mreportsgroups.active as active_group,name as group_name, group_slug');
        $this->db->from('mreports');
        $this->db->join('mreportsgroups', 'mreportsgroups.groupid = mreports.articlegroup', 'left');
    ...
    public function add_article($data)
    {
        ...
        $slug_total = $this->db->count_all_results('mreports');
        if ($slug_total > 0) {
    ...
    {
        $this->db->where('articleid', $id);
        $this->db->delete('mreports');
        if ($this->db->affected_rows() > 0) {

            // $this->db->where('rel_type', 'mreports_article');
    ...
    public function get_kbg($id = '', $active = '')
    {
        ...
        if (is_numeric($id)) {
            ...
            return $this->db->get('mreportsgroups')->row();
        }
        $this->db->order_by('group_order', 'asc');

        return $this->db->get('mreportsgroups')->result_array();
    }
    ...
    public function add_group($data)
    {
        ...
        $slug_total = $this->db->count_all_results('mreportsgroups');
        if ($slug_total > 0) {
            $data['group_slug'] .= '-' . ($slug_total + 1);
        }

        $this->db->insert('mreportsgroups', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
    ...
    public function update_group($data, $id)
    {
        ...
        $this->db->where('groupid', $id);
        $this->db->update('mreportsgroups', $data);
        if ($this->db->affected_rows() > 0) {
    ...
    public function delete_group($id)
    {
        ...
        if (is_reference_in_table('articlegroup', 'mreports', $id)) {
            return [
                'referenced' => true,
            ];
        }
        $this->db->where('groupid', $id);
        $this->db->delete('mreportsgroups');
        if ($this->db->affected_rows() > 0) {
