<div class="modal fade" id="laws_group_modal" tabindex="-1" role="dialog">
    <div class="modal-dialog">
        <?php echo form_open(admin_url('laws/group'),array('id'=>'laws_group_form')); ?>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <?php echo render_input('group_order','kb_group_order',total_rows('lawsgroups') + 1,'number'); ?>
...
<script>
    ...
    _validate_form($('#laws_group_form'), {
        name: 'required'
    }, manage_kb_groups);

    // On hidden modal reset the values
    $('#laws_group_modal').on("hidden.bs.modal", function(event) {
        $('#kb_group_slug').addClass('hide');
        $('#kb_group_slug input').rules('remove', 'required');
        $('#additional').html('');
        $('#laws_group_modal input').not('[type="hidden"]').val('');
        $('#laws_group_modal textarea').val('');
        $('.add-title').removeClass('hide');
        $('.edit-title').removeClass('hide');
        $('#laws_group_modal input[name="group_order"]').val($('table tbody tr').length + 1);
    });
});
...
function manage_kb_groups(form) {
    ...
    var url = form.action;
    var articleAddEdit = $('body').hasClass('laws-article');
    if(articleAddEdit) {
...
function new_laws_group() {
    $('#laws_group_modal').modal('show');
...
function edit_kb_group(invoker, id) {
    ...
    $('#laws_group_modal input[name="name"]').val($(invoker).data('name'));
    $('#laws_group_modal textarea[name="description"]').val($(invoker).data('description'));
    $('#laws_group_modal input[name="group_order"]').val($(invoker).data('order'));
