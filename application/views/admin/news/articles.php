<?php init_head();
$has_permission_edit = has_permission('news','','edit');
$has_permission_create = has_permission('news','','create');
?>
<div id="wrapper">
  <div class="content">
    <div class="row">
         ...
         <div class="panel-body">
           <div class="_buttons">
             <?php if($has_permission_create){ ?>
               <a href="<?php echo admin_url('news/article'); ?>" class="btn btn-info mright5"><?php echo _l('kb_article_new_article'); ?></a>
               <?php } ?>
               <?php if($has_permission_edit || $has_permission_create){ ?>
                 <a href="<?php echo admin_url('news/manage_groups'); ?>" class="btn btn-info mright5"><?php echo _l('als_kb_groups'); ?></a>
                 <?php } ?>
             ...
             <div class="row">
               <div class="tab-content">
                 <div role="tabpanel" class="tab-pane kb-kan-ban kan-ban-tab" id="kan-ban">
                        ...
                        <div class="border-right panel_s">
                         <div class="panel-heading-bg primary-bg" style="background: #000" data-group-id="<?php echo $group['groupid']; ?>">
                          <?php if($has_permission_edit){ ?>
                            ...
                            <small> - <?php echo total_rows('news','articlegroup='.$group['groupid']); ?></small>
                            </div>
                            <?php
                            $this->db->select('*, (SELECT COUNT(*) FROM tblviewstracking WHERE rel_type="news_article" AND rel_id=news.articleid) as total_views')->from('news')->where('articlegroup',$group['groupid'])->order_by('article_order','asc');
                               ...
                               <ul class="sortable article-group groups<?php if(!$has_permission_edit){echo 'sortable-disabled'; } ?>" data-group-id="<?php echo $group['groupid']; ?>">
                                <?php foreach($articles as $article) { ?>
                                  <li class="<?php if($article['active'] == 0){echo 'line-throught';} ?>" data-article-id="<?php echo $article['articleid']; ?>">
                                   <div class="panel-body">
                                          <a href="<?php echo site_url(NEWS_SLUG.$article['slug']); ?>" target="_blank">
                                           <?php echo $article['subject']; ?></a>
                                            <?php if($has_permission_edit){ ?>
                                              <a href="<?php echo admin_url('news/article/'.$article['articleid']); ?>" target="_blank" class="pull-right"><span><i class="fa fa-pencil-square-o" aria-hidden="true"></i></span></a>
                                              <?php } ?>
                  ...
                  <?php include_once(APPPATH.'views/admin/news/group.php'); ?>
                  <?php init_tail(); ?>
                  <script>
                   ...
                   $(function(){
                     ...
                     update: function(event, ui) {
                       if (this === ui.item.parent()[0]) {
                         ...
                         setTimeout(function() {
                           $.post(admin_url + 'news/update_kan_ban', {
                             order: order,
                     ...
                     $(".container-fluid").sortable({
                       ...
                       update: function(event, ui) {
                         ...
                         var data = {}
                         data.order = order;
                         $.post(admin_url + 'news/update_groups_order', data);
                       }
                     });
